...
class CreatePerformances extends Migration
{
    ...
    {
        $this->down();
        Schema::connection(config('performance.connection'))->create(Performance::getTableName(), function (Blueprint $table) {
            $table->increments('id');
            $table->string('uid');
            $table->string('url');
            $table->string('uri');
    ...
    {
        Schema::connection(config('performance.connection'))->dropIfExists(Performance::getTableName());
    }
}
